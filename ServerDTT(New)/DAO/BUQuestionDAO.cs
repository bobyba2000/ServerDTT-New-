...
namespace ServerDTT_New_.DAO
{
    class BUQuestionDAO
    {
        private static BUQuestionDAO instance;
        public static BUQuestionDAO Instance
        {
            get
            {
                if (instance == null)
                    instance = new BUQuestionDAO();
                return instance;
            }
        }
        public BUQuestionDAO()
        {
            //
        }
        ...
        {
            string query = string.Format(
                "DELETE FROM BUQUESTION " +
                "WHERE BUQuestionID = {0}", questionID);
            int result = DataProvider.Instance.ExecuteNonQuery(query);
            return result > 0;
        }
    }
